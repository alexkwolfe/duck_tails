...
usage() {
    ...
    echo "Commands:"
    echo "  setup    - Extract fixtures to temporary directory"
    ...
    echo "  DUCK_TAILS_TEST_FIXTURES_DIR - Override temp directory location"
}

# Function to setup fixtures
setup_fixtures() {
    # Create temporary directory
    if [ -n "$DUCK_TAILS_TEST_FIXTURES_DIR" ]; then
        TEMP_DIR="$DUCK_TAILS_TEST_FIXTURES_DIR"
        mkdir -p "$TEMP_DIR"
    else
        TEMP_DIR=$(mktemp -d -t duck_tails_test_XXXXXX)
    fi
